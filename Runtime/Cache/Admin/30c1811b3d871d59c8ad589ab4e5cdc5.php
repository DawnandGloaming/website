...
<body>
    ...
    <form role="form" method="post" action="<?php echo U('Navigation/editor');?>" class="create_page valid_form">
        <div class="form-group">
            <label>上级导航</label>
            <select name="pid" class="form-control">
                <?php if(is_array($navigation_list)): $i = 0; $__LIST__ = $navigation_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $navigation_info['pid']): ?>selected = "selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
        <div class="form-group">
            <label>导航名称</label>
            <input type="text" name="name" value="<?php echo ($navigation_info["name"]); ?>" class="form-control" autocomplete="off" placeholder="请输入导航名称">
        </div>
        <div class="form-group">
            <label>导航链接</label>
            <input type="text" name="url" value="<?php echo ($navigation_info["url"]); ?>" class="form-control" autocomplete="off" placeholder="请输入导航链接">
        </div>
        <div class="form-group">
            <label></label>
            <input type="hidden" name="id" value="<?php echo ($navigation_info["id"]); ?>">
