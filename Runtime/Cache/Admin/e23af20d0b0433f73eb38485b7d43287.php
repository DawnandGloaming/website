...
<body>
    ...
    <ol class="breadcrumb admin_nav_bread">
        <button type="button" class="btn btn-sm btn-success create_layer" url="<?php echo U('Group/create');?>" layer-w="450px;" layer-h="350px;">添加分组</button>
    </ol>
    <table class="table table-hover table-condensed table_list">
        ...
        <tbody>
        <?php if(empty($group_list)): ?><tr>
                <td align="center" colspan="6">
                    <span style="color: red;">暂无数据</span>
                </td>
            </tr>
            <?php else: ?>
            <?php if(is_array($group_list)): $i = 0; $__LIST__ = $group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td align="center"><?php echo ($vo["name"]); ?></td>
                    <td align="center"><?php echo ($vo["intro"]); ?></td>
                    <td align="center">
                        <?php if($vo["status"] == 1): ?><span class="ajax_get" url="<?php echo U('change_status',array('type'=>'status','status'=>$vo['status'],'id'=>$vo['id']));?>" style="color: green;cursor:pointer;">开启</span>
                            <?php else: ?>
                            <span class="ajax_get" url="<?php echo U('change_status',array('type'=>'status','status'=>$vo['status'],'id'=>$vo['id']));?>" style="color: red;cursor:pointer;">关闭</span><?php endif; ?>
                    </td>
                    <td align="center">
                        <button type="button" class="btn btn-sm btn-info create_layer" url="<?php echo U('Group/editor',array('id'=>$vo['id']));?>" layer-w="450px;" layer-h="350px;">修改</button>
                        <a href="<?php echo U('Group/give_rule',array('id'=>$vo['id']));?>"><button type="button" class="btn btn-sm btn-default">授权</button></a>
                        <button type="button" class="btn btn-sm btn-danger ajax_get confirm" url="<?php echo U('Group/remove',array('id'=>$vo['id']));?>">删除</button>
                    </td>
