...
<body>
    ...
    <ol class="breadcrumb admin_nav_bread">
        <button type="button" class="btn btn-sm btn-success create_layer" url="<?php echo U('Navigation/create',array('pid'=>$_GET['pid']));?>" layer-w="450px;" layer-h="400px;">添加导航</button>
        <button type="button" class="btn btn-sm btn-primary list_save">保存操作</button>
    </ol>
    <form action="<?php echo U('Navigation/list_form');?>" method="post" class="valid_form list_form">
        <table class="table table-hover table-condensed table_list">
            ...
            <tbody>
                <?php if(empty($navigation_list)): ?><tr>
                        <td align="center" colspan="6">
                            <span style="color: red;">暂无数据</span>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php if(is_array($navigation_list)): $i = 0; $__LIST__ = $navigation_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td align="center">
                            ...
                            <td align="center">
                                <?php if(($vo["pid"]) == "0"): ?><a href="<?php echo U('Navigation/index',array('pid'=>$vo['id']));?>"><?php echo ($vo["name"]); ?></a>
                                <?php else: ?>
                                    <?php echo ($vo["name"]); endif; ?>
                            </td>
                            <td align="center">
                                <?php echo ($vo["pid_name"]); if(empty($vo["pid_name"])): ?>顶级导航<?php endif; ?>
                            </td>
                            <td align="center">
                                <?php if($vo["is_show"] == 1): ?><span class="ajax_get" url="<?php echo U('change_status',array('type'=>'is_show','is_show'=>$vo['is_show'],'id'=>$vo['id']));?>" style="color: green;cursor:pointer;">是</span>
                                <?php else: ?>
                                    <span class="ajax_get" url="<?php echo U('change_status',array('type'=>'is_show','is_show'=>$vo['is_show'],'id'=>$vo['id']));?>" style="color: red;cursor:pointer;">否</span><?php endif; ?>
                            </td>
                            <td align="center"><?php echo ($vo["url"]); ?></td>
                            <td align="center">
                                <button type="button" class="btn btn-sm btn-info create_layer" url="<?php echo U('Navigation/editor',array('id'=>$vo['id']));?>" layer-w="450px;" layer-h="400px;">修改</button>
                                <button type="button" class="btn btn-sm btn-danger ajax_get confirm" url="<?php echo U('Navigation/remove',array('id'=>$vo['id']));?>">删除</button>
                            </td>
